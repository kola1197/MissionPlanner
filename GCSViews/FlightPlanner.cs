...
namespace MissionPlanner.GCSViews
{
    public partial class FlightPlanner : MyUserControl, IDeactivate, IActivate
    {
        ...
        internal GMapPolygon DrawingPolygon;
        ...
        Locationwp rallyWp = new Locationwp();
        ...
        public void Init()
        {
            ...
            DrawingPolygon = new GMapPolygon(polygonPoints2, "drawnpoly");
            DrawingPolygon.Stroke = new Pen(Color.Red, 2);
            DrawingPolygon.Fill = Brushes.Transparent;
            ...
            new Thread(delegate()
            {
                try
                {
                    ...
                    if (wpLoadingActive != 2)
                    {
                        Task.Run(async () =>
                        {
                            await mav_mission.upload(MainV2.comPort, MainV2.comPort.MAV.sysid,
                                MainV2.comPort.MAV.compid,
                                type,
                                commandlist,
                                (percent, status) =>
                    ...
                    else
                    {
                        ...
                        if (rallyWp.lng != 0 || rallyWp.lat != 0)
                        {
                            MainV2.comPort.setParam((byte) MainV2.comPort.sysidcurrent,
                                (byte) MainV2.comPort.compidcurrent, "RALLY_TOTAL", 3);
                            // TODO: may be array of raly points?
                            try
                            {
                                MainV2.comPort.setRallyPoint(count,
                                    new PointLatLngAlt(rallyWp.lat, rallyWp.lng) {Alt = rallyWp.alt}, 0, 0, 0,
        ...
        public void callMeDrag(string pointno, double lat, double lng, int alt)
        {
            ...
            {
                // auto update home alt
                rallyWp.alt = (float) (srtm.getAltitude(lat, lng).alt * CurrentState.multiplieralt);

                rallyWp.lat = lat;
                rallyWp.lng = lng;
                System.Diagnostics.Debug.WriteLine("rallyWP 1019( callMeDrag ): " + rallyWp.lat.ToString() + ", " +
                                                   rallyWp.lng);
        ...
        //     {
        //         tag++;
        //         DrawingPolygon.Points.Add(x);
        //         addpolygonmarkergrid(tag.ToString(), x.Lng, x.Lat, 0);
        //     });
        //
        //     DrawnPolygonsOverlay.Polygons.Add(DrawingPolygon);
        //     MainMap.UpdatePolygonLocalPosition(DrawingPolygon);
        //
        //     {
        //         foreach (var pointLatLngAlt in DrawingPolygon.Points.CloseLoop().PrevNowNext())
        //         {
        ...
        public void setfromMap(double lat, double lng, int alt, double p1 = -1)
        {
            ...
            {
                if (CHK_verifyheight.Checked && (altmode) CMB_altmode.SelectedValue != altmode.Terrain
                ) //Drag with verifyheight // use srtm data
                {
        ...
        public void writeKML()
        {
            ...
            try
            {
                ...
                if ((MAVLink.MAV_MISSION_TYPE) cmb_missiontype.SelectedValue == MAVLink.MAV_MISSION_TYPE.RALLY || true)
                {
                    var overlay = new WPOverlay();
                    ...
                    {
                        overlay.CreateOverlay(PointLatLngAlt.Zero,
                            commandlist1, 0, 0, CurrentState.multiplieralt);
                    }
                    catch (FormatException ex)
                    {
                        CustomMessageBox.Show(Strings.InvalidNumberEntered, Strings.ERROR);
        ...
        public void addPolygonPointToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            //     DrawingPolygon.Points[0] == DrawingPolygon.Points[DrawingPolygon.Points.Count - 1])
            //     DrawingPolygon.Points.RemoveAt(DrawingPolygon.Points.Count - 1); // unmake a full loop
            //
            // DrawingPolygon.Points.Add(new PointLatLng(MouseDownStart.Lat, MouseDownStart.Lng));
        ...
        {
            double aream2 = Math.Abs((double) calcpolygonarea(DrawingPolygon.Points));
        ...
        private AircraftConnectionInfo GetAircraftByMavState(MAVState mavState)
        {
        ...
        private string GetAircraftNum(AircraftConnectionInfo aircraft)
        {
            foreach (var keyValuePair in MainV2.Aircrafts)
            {
                if (keyValuePair.Value == aircraft)
                {
        ...
        // Draw Aircraft Num, GroundSpeed, Alt of all connected MAVs 
        private void DrawMAVInfo(PaintEventArgs e, MAVState mavState)
        {
            AircraftConnectionInfo aircraft = GetAircraftByMavState(mavState);
            if (aircraft == null)
            {
                return;
            }
        ...
        private void DrawMavInfoString(PaintEventArgs e, Rectangle rectAirNum, string aircraftNum, Color inlineColor,
            Color outlineColor)
        {
        ...
        {
            int count = 0;
            DrawingPolygon.Points.ForEach(a => AddCommand(MAVLink.MAV_CMD.FENCE_POLYGON_VERTEX_EXCLUSION,
                DrawingPolygon.Points.Count, 0, 0, 0, a.Lng, a.Lat, count++));
        ...
        {
            int count = 0;
            DrawingPolygon.Points.ForEach(a => AddCommand(MAVLink.MAV_CMD.FENCE_POLYGON_VERTEX_INCLUSION,
                DrawingPolygon.Points.Count, 0, 0, 0, a.Lng, a.Lat, count++));
                    ...
                    try
                    {
                        ...
                        foreach (var feature in fs.Features)
                        {
                            foreach (var point in feature.Coordinates)
                            {
                                ...
                                DrawingPolygon.Points.Add(new PointLatLng(point.Y, point.X));
                                addpolygonmarkergrid(DrawingPolygon.Points.Count.ToString(), point.X, point.Y, 0);
                            }
        ...
        public void GeoFenceuploadToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            List<PointLatLng> plll = new List<PointLatLng>(DrawingPolygon.Points.ToArray());
            // close it
            plll.Add(plll[0]);
            // check it
            if (
            ...
            // points + return + close
            byte pointcount = (byte) (DrawingPolygon.Points.Count + 2);
            ...
            try
            {
                ...
                // add polygon close
                MainV2.comPort.setFencePoint(a, new PointLatLngAlt(DrawingPolygon.Points[0]), pointcount);
                ...
                // add polygon
                geofencepolygon.Points.AddRange(DrawingPolygon.Points.ToArray());
                ...
                if (File.Exists(fd.FileName))
                {
                    ...
                    while (!sr.EndOfStream)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                DrawingPolygon.Points.Add(new PointLatLng(
        ...
        public void MainMap_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (Commands.Rows.Count > 0 && wpMenu1.IsWpDistanceDrawing)
            {
 ...
 Column 7: Flags (just 0 for now, also future use).
        ...
        private void setWPParams()
        {
            try
            {
                ...
                {
                    TXT_WPRad.Text = (((double) param["WPNAV_RADIUS"] * CurrentState.multiplierdist / 100.0))
                        .ToString();
                }
        ...
        private bool IsMAVInAircraftsList(MAVLinkInterface mav)
            ...
            private void MainMap_MouseDown(object sender, MouseEventArgs e)
            {
                ...
                if ((e.Button == MouseButtons.Left || e.Button == MouseButtons.Right) &&
                    (groupmarkers.Count > 0 || Control.ModifierKeys == Keys.Control))
                {
                    // group move
                    isMouseDown = true;
            ...
            public double[] getWPCoords(int serialIndex) // 
            {
                ...
                for (int i = 0; i < Commands.Rows.Count; i++)
                {
                    ...
                    if ((cmd == (ushort) MAVLink.MAV_CMD.WAYPOINT || cmd == (ushort) MAVLink.MAV_CMD.LOITER_TIME) &&
                        getWPSerialNumber(i) == serialIndex)
                    {
            ...
            private void MainMap_MouseDoubleClick(object sender, MouseEventArgs e)
            {
                if (e.Button == MouseButtons.Left)
                {
                    if (CurrentRallyPt != null)
                    {
                        ...
                        wpConfig.controller = new NewClasses.UniversalCoordinatsController(
                            new NewClasses.WGSCoordinats(pointlist[0].Lat.ToString(), pointlist[0].Lng.ToString()));
                    ...
                    else if (CurentRectMarker != null && (CurentRectMarker.Overlay != RegionsOverlay) &&
                             CurentRectMarker.Overlay != RulerOverlay)
                    {
                        ...
                        else
                        {
                            wpConfig = new WPConfig(CurentRectMarker,
                                getWPSerialNumber(int.Parse(CurentRectMarker.Tag.ToString()) - 1).ToString());
                            WpConfigAddEvents();
            ...
            private void WpConfig_FormClosing(object sender, FormClosingEventArgs e)
            {
                if (wpConfig.closedByButton)
                {
                    ...
                    else if (selectedValue == 5)
                    {
                        ...
                        System.Diagnostics.Debug.WriteLine("rallyWP ( FromClosing ): " + rallyWp.lat.ToString() + ", " +
                                                           rallyWp.lng);
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (!wpConfig.checkBox1.Checked)
                            {
                                ...
                                ) //Домашняя точка, Точка взлета, Маршрутная точка, Изменение скорости, Точка посадки
                                {
                                    ...
                                    case 3:
                                        ...
                                        row.Cells[Command.Index + 2].Value =
                                            speed.ToString(); //String.Format("{0:0.00}", (speed / 3.6));
                            ...
                            {
                                Commands.Rows[index].Cells[Command.Index].Value =
                                    MAVLink.MAV_CMD.LOITER_TIME.ToString();
                                double t = 0;
                                double.TryParse(wpConfig.textBox3.Text, out t);
                                t *= 60;
                                Commands.Rows[index].Cells[Command.Index + 1].Value = t.ToString();
                                Commands_CellUpdate(index, Command.Index + 1);
                            }
            ...
            {
                wpConfig.controller = new NewClasses.UniversalCoordinatsController(new NewClasses.WGSCoordinats(
                    Commands.Rows[wpConfig.indexNow].Cells[Lat.Index].Value.ToString(),
                ...
                wpConfig.wpAltSlidingScale1.alt_SlidingScale.Value =
                    Convert.ToDouble(Commands.Rows[index].Cells[Alt.Index].Value);
                ...
                switch (cmd) //Точка взлета, Маршрутная точка, Изменение скорости, Точка посадки
                {
                    ...
                    case (ushort) MAVLink.MAV_CMD.LOITER_TIME:
                        ...
                        double t = 0;
                        double.TryParse(Commands.Rows[wpConfig.indexNow].Cells[Command.Index + 1].Value.ToString(),
                            out t);
                        t /= 60;
                        wpConfig.textBox3.Text = Math.Round(t).ToString();
                        break;
                    default:
                        wpConfig.comboBox1.SelectedIndex = 2;
                        break;
            ...
            private void MainMap_MouseUp(object sender, MouseEventArgs e)
            {
                ...
                if (isMouseDown) // mouse down on some other object and dragged to here.
                {
                    ...
                    if (CurrentMidLine is GMapMarkerPlus)
                    {
                        ...
                        {
                            // var idx = DrawingPolygon.Points.IndexOf(midline.now);
                            // DrawingPolygon.Points.Insert(idx + 1,
                        ...
                        else
                        {
                            if (int.TryParse(midline.next.Tag, out pnt2))
                            {
                                    ...
                                    MAVLink.MAV_MISSION_TYPE.FENCE)
                                {
                                    ...
                                    InsertCommand(pnt2 - 1,
                                        (MAVLink.MAV_CMD) Enum.Parse(typeof(MAVLink.MAV_CMD), prevtype),
                                        0, 0, 0, 0,
                    ...
                    {
                        if (isMouseDraging && CurrentRallyPt != null)
                        {
                            rallyWp.lat = MouseDownEnd.Lat;
                            rallyWp.lng = MouseDownEnd.Lng;
                            System.Diagnostics.Debug.WriteLine("rallyWP 7910( MainMap_MouseUp ): " +
                                                               rallyWp.lat.ToString() + ", " + rallyWp.lng);
                            CurentRectMarker = null;
                            CurrentRallyPt = null;
                        }
                        else if (groupmarkers.Count > 0)
        ...
        private void writeOtherWPtoWPConfig(int index)
        {
                    ...
                    (ushort) MAVLink.MAV_CMD.LOITER_TIME))
            {
                ...
                switch (cmd)
                {
                    ...
                    case (ushort) MAVLink.MAV_CMD.DO_CHANGE_SPEED:
                        wpConfig.comboBox1.SelectedIndex = 3;
                        string s = Commands.Rows[index + 1].Cells[Command.Index + 2].Value.ToString();
                        double speed = double.Parse(s);
                        speed *= 3.6;
                        wpConfig.textBox5.Text = Math.Round(speed).ToString();
                        break;
        ...
        private bool cleanToPrevWP(int index)
        {
            bool result = false;
            if (index > 0 &&
                (ushort) Enum.Parse(typeof(MAVLink.MAV_CMD),
        ...
        public void tryToWriteWP()
        {
                ...
                MainV2.CurrentAircraft.UsingSitl)
            {
                for (int i = 0; i < Commands.Rows.Count; i++)
                {
                    if ((ushort) Enum.Parse(typeof(MAVLink.MAV_CMD),
                        Commands.Rows[i].Cells[Command.Index].Value.ToString(), false) == (ushort) MAVLink.MAV_CMD.LAND)
                    {
                        int k = i;
                        bool b = false;
                        while (k > 0 && !b)
                        {
                            k--;
                            if ((ushort) Enum.Parse(typeof(MAVLink.MAV_CMD),
                                    Commands.Rows[k].Cells[Command.Index].Value.ToString(), false) ==
                                (ushort) MAVLink.MAV_CMD.WAYPOINT)
                            {
                                b = true;
